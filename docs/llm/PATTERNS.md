...
| JabKit installed block | `apps/web/src/components/jabkit/<name>/` | `@/components/jabkit/...` |
...
Never re-create `apps/*/src/components/` for something shared — it belongs in `packages/ui`. **Exception:** `@jabkit/cli` may write only `apps/web/src/components/jabkit/`. Domain UI stays in `sections/` and `modules/`. Admin never grows `src/components/`. CLI recipe: [`ROADMAP/00-conventions.md`](../../ROADMAP/00-conventions.md).

### JabKit exception (`apps/web/src/components/jabkit/` only)

The only allowed `apps/web/src/components/` tree is `jabkit/`, generated by `@jabkit/cli`. Do not hand-author Playmates widgets there. `apps/admin` must not gain a `src/components/` tree.
