...
import type { FailedJobsProps } from "./FailedJobs.types";

function FailedJobs({ rows }: FailedJobsProps) {
  return (
    <Card size="sm" data-slot="failed-jobs" className="gap-0 bg-background text-foreground">
      ...
      <CardContent className="px-(--card-spacing) py-3">
        {rows.length === 0 ? (
          <Empty className="min-h-0 p-4">
            <EmptyHeader>
              <EmptyTitle className="font-normal text-muted-foreground">No failed jobs</EmptyTitle>
            </EmptyHeader>
          </Empty>
        ) : (
            ...
            {rows.map((row) => (
              <li key={row.id} className="flex min-w-0 flex-col gap-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <StatusBadge kind="upload-job" status="failed" />
                  <p className="font-mono text-xs text-destructive">{row.errorCode}</p>
                </div>
              </li>
            ))}
          </ul>
